...
for job in range(n_jobs):
  with open("PSA_WNV_%03d.sbatch" % (job), 'w') as f:
    ...
    f.write("python3 WNV_sim_sherlockPSA.py %d %d %d PSA_WNV_%03d.csv" % (iter_start, iters_per_job, N_rep, job))
  
  os.system("sbatch PSA_WNV_%03d.sbatch" % (job))
  time.sleep(1)
