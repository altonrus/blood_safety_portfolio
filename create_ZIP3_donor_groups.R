...
## ASIDE - look at cases by month
tti_yield_confirmed <- tti_yield[interpretation_simp == "Confirmed"]
ggplot(tti_yield_confirmed)+
  geom_histogram(aes(x = factor(month), fill = season), stat = "count")+
  facet_grid(rows = vars(year), cols = vars(TTI))+
  scale_x_discrete(labels = c("Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul",
                            "Aug", "Sep", "Oct", "Nov", "Dec"),
                   name = "Month of donation")+
  scale_y_continuous(name = "Confirmed positive donations")+
  theme_bw()+
  theme(legend.position = "bottom")

ggsave("./manuscript/fig_yield_by_season.png",
       width = 6.5,
       height = 6,
       units = "in")


#tti_yield[ , sum(n_cases), by = c("TTI", "year", "")]
